...
- Added detailed Used, Available, Buffered and Cached memory values to the Overview and Router Performance cards, using the same normalized values as Router System.
- Applied refreshed memory snapshots together across RouterPilot so memory values and percentage update promptly after an existing refresh without requiring navigation or a separate memory poll.
- Improved the Router Performance Memory / Storage layout with labelled storage values, responsive sizing and clearer spacing beside the scrollbar.
