...
public partial class PluginPackagesViewModel : ObservableObject
{
    ...
    public async Task RemoveSelectedAsync() => await MutateAsync("remove", "Removing…", "Removed successfully.");
    ...
    private async Task MutateAsync(string operation, string working, string success)
    {
        ...
        if (operation == "install" && !CanInstall || operation == "remove" && !CanRemove || operation == "update-indexes" && !CanRefreshIndexes) return;
    ...
    private void NotifyActionState() { OnPropertyChanged(nameof(CanInstall)); OnPropertyChanged(nameof(CanRemove)); OnPropertyChanged(nameof(CanRefreshIndexes)); }
