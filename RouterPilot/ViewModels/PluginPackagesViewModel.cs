...
public partial class PluginPackagesViewModel : ObservableObject
{
    ...
    [RelayCommand(CanExecute = nameof(CanInstall))] private Task InstallAsync() => MutateAsync("install", "Installing…", "Installed successfully.");
    [RelayCommand(CanExecute = nameof(CanRemove))] private Task RemoveAsync() => MutateAsync("remove", "Uninstalling…", "Uninstalled successfully.");
    [RelayCommand(CanExecute = nameof(CanUpdate))] private Task UpdateAsync() => MutateAsync("update-package", "Updating…", "Updated successfully.");
    [RelayCommand(CanExecute = nameof(CanRefreshIndexes))] private Task RefreshIndexesAsync() => MutateAsync("update-indexes", "Refreshing package indexes…", "Package indexes refreshed.");
    private async Task MutateAsync(string operation, string working, string success)
    {
        ...
        if (selected is not null && operation != "update-indexes" && !ConfirmMutation(operation, selected)) return;
    ...
    private static bool ConfirmMutation(string operation, PluginPackage package)
    {
        ...
        string detail = operation == "update-package" ? $"\n\nInstalled: {package.DisplayInstalledVersion}\nAvailable: {package.DisplayAvailableVersion}" : "\n\nThis changes software installed on the router.";
        return MessageBox.Show($"{action} {package.Name}?{detail}", $"{action} plug-in", MessageBoxButton.YesNo, operation == "remove" ? MessageBoxImage.Warning : MessageBoxImage.Question) == MessageBoxResult.Yes;
    }
